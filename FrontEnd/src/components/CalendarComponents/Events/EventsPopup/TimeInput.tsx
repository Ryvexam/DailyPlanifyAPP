...
const TimeInput: React.FC<TimeInputProps> = ({ label, value = '00:00', onChange, required, readOnly }) => {
  const handleTimeChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    const newTime = `${event.target.name === 'hour' ? event.target.value : hour}:${event.target.name === 'minute' ? event.target.value : minute}`;
    onChange(newTime);
  };
    ...
    <div className="flex-1">
      ...
      <div className="flex space-x-2">
        ...
        <select
          ...
          value={minute}
          onChange={handleTimeChange}
          className="w-full rounded-sm border border-stroke bg-white px-4.5 py-3 text-black focus:border-primary focus-visible:outline-none dark:border-strokedark dark:bg-boxdark dark:focus:border-primary"
          required={required}
          disabled={readOnly}
        >
